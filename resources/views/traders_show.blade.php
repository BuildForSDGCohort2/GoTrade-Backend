...
<div class="row">
	<div class="col-md-8">
		<form method="post"
			action="{{ route('trader_profile.save',['trader'=>$trader]) }}">
			@csrf
			<div class="card">
					...
					<div class="form-row">
						<div class="form-group col-md-6">
							<label for="inputFirstName">First Name</label> <input type="text"
								...
								value="{{ $trader->first_name }}">
						</div>
						<div class="form-group col-md-6">
							<label for="inputLastName">Last Name</label> <input type="text"
								class="form-control" value="{{ $trader->last_name }}"
					...
					<div class="form-row">
						<div class="form-group col-md-6">
							<label for="inputGender">Gender</label> <select
								...
								<?php if($trader->gender==null) {?>
								<option value="">Select Gender</option>
								<?php } else { ?>
								<option value="{{ $trader->gender  }}"><?= $trader->gender=='male'?'MALE':'FEMALE' ?></option>
								<?php } ?>
						...
						<div class="form-group col-md-6">
							...
							<div class="input-group">
								...
								<input type="text" class="form-control past-now-date"
									...
									value="{{ $trader->date_of_birth }}" readonly="true">
							</div>
						</div>
					</div>
					<div class="form-row">
					...
					<div class="form-row">
						<div class="form-group col-md-4">
							<label for="inputCountry">Country</label> <select
								...
								<?php if($trader->country_id==null) {?>
								<option value=""></option>
								<?php } else { ?>
								<option value="{{ $trader->country->id  }}"><?= $trader->country->name ?></option>
								<?php } ?>						
						...
						<div class="form-group col-md-4">
							<label for="inputState">State</label> <select id="inputState"
								...
								<?php if($trader->state_id==null) {?>
								<option value=""></option>
								<?php } else { ?>
								<option value="{{ $trader->state->id  }}"><?= $trader->state->name ?></option>
								<?php } ?>						
						...
						<div class="form-group col-md-4">
							<label for="inputCity">City</label> <select id="inputCity"
								...
								<?php if($trader->city_id==null) {?>
								<option value=""></option>
								<?php } else { ?>
								<option value="{{ $trader->city->id  }}"><?= $trader->city->name ?></option>
								<?php } ?>
	...
	<div class="col-md-4">
		<form method="post"
			action="{{ route('trader_profile.change_photo',['trader'=>$trader]) }}"
			...
			<div class="card">
				...
				<div class="card-body">
					<img
						src="{{ route('get_profile_photo',['filename'=>md5($trader->id)]) }}"
						alt="{{$trader->email}}" class="img-thumbnail img-fluid"
						width="100%">
					<hr>
		...
		<form method="post"
			action="{{ route('trader_profile.change_password',['trader'=>$trader]) }}">
			@csrf
			<div class="card">
				<div class="card-header">
		...
		<form method="post"
			action="{{ route('trader_profile.update_status',['trader'=>$trader]) }}">
			@csrf
			<div class="card">
				<div class="card-header">
					<h3 class="card-title">Account Status: {{
						\App\Services\TraderService::getStatusMessage($trader->status)
						}}</h3>
				</div>
				<div class="card-footer">
